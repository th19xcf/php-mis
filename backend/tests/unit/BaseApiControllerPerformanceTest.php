...
/**
 * buildPerformanceTable 特征测试（重构安全网）
 *
 * 逐字锁定性能追踪表格日志的输出格式：首行/表头/分隔行/数据行/耗时排行。
 * 方法为纯字符串转换，无 DB / 无请求态依赖。
 */
class BaseApiControllerPerformanceTest extends CIUnitTestCase
{
    ...
    public function testMultiStepOutputExactLines(): void
    {
        ...
        $out = $this->controller->buildPerformanceTableEx('[Test]', '成功', 'user=xx', $steps, $t0);
        $lines = explode("\n", $out);
    ...
    public function testEmptyStepsClampsNegativeTotalToMinimum(): void
    {
        ...
        $out = $this->controller->buildPerformanceTableEx('[T]', '成功', 'info', [], 1_000_000_000);
        $lines = explode("\n", $out);
    ...
    public function testTotalBelowOneMicrosecondClamped(): void
    {
        ...
        $out = $this->controller->buildPerformanceTableEx('[T]', '成功', 'i', $steps, $t0);
    ...
    public function testSingleStepRowAndRanking(): void
    {
        ...
        $out = $this->controller->buildPerformanceTableEx('[Tag]', '失败', 'fc=2025', $steps, $t0);
        $lines = explode("\n", $out);
