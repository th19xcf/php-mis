...
import { fetchUpdateEmployee, fetchBatchUpdateEmployee, fetchDeleteEmployee, fetchEmployeeDebugTree } from '@/service/api';
...
async function handleDebugTree() {
  try {
    const { data } = await fetchEmployeeDebugTree();
    if (!data) {
      message.error('调试信息为空');
      return;
    }
    console.group('%c[调试] 员工树 SQL 追踪', 'color: #fa8c16; font-weight: bold');
